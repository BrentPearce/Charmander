...
protected:
	double * domain;
	bool leftBdryIsDirichlet; // value is "true" if left BC is Dirichlet
	bool rightBdryIsDirichlet; // value is "true" if right BC is Dirichlet
	bool leftBdryIsPeriodic;// value is "true" if the left BC is periodic
	bool rightBdryIsPeriodic;// value is "true" if the  BC is periodic
	double * leftBdryValues; // contains gamma_0 and g_0 
	double * rightBdryValues; // contains gamma_0 and g_0 
	bool reactionIsPresent; // value is "true" if there is a reaction
	bool forcingFunctIsPresent; // value is "true if there is a forcing f(x)
...
public:
	...
	//Set TwoPointBVP prob object to have periodic BCs;
	void set_left_periodic_bdry();//ie right is defined & non-dirichlet

	void set_right_periodic_bdry();//ie left is defined & non-dirichlet
	...
	********************/
	...
	bool is_left_bdry_periodic() const;

	bool is_right_bdry_periodic() const;
