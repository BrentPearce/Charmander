...
double truesol(vector<double> & x)
{
	double c1 = -9.0/16.0;
	double c2 = -7.0 / 80.0;
	//double c2 = -9.0 / 80; // if it is left PBC

	return -x[0] * x[0] * x[0] * x[0] / 4.0 - c1 * x[0] * x[0] * x[0] / 3 -
		x[0] * x[0] / 2.0 - c1 * x[0] + c2;
}
...
int main()
{
	...
	lbval[0] = -5.0; // this is gamma_l
	lbval[1] = 1.0; // this is g_l
	prob->set_left_bdry(false, lbval);


	//double *rbval = new double[2];
	//rbval[0] = -5.0; //this is gamma_r
	//rbval[1] = 1.0;//this is g_r
