...
vector<double> TwoPointBVPAppr::Solve(int max_num_iter, double TOL)
{
	...
	else if (theproblem->is_right_bdry_periodic()) 
	{
		...
		for (int iter = 1; iter <= max_num_iter; iter++)
		{
			...
			//Add U_0*P_hat to B(U_hat)
			GStar[numsubintervals-1] = GStar[numsubintervals-1]
				- U[numsubintervals] * A_NM1_N;
...
vector<double> TwoPointBVPAppr::find_errors(int max_iters, double TOL)
{
	...
	// durring the sweep. Also save the approx error and output it.

	// create error vector and intialize max error
	double max_error =-1 ;
	vector<double> approxErr(numberSubintervals);

	//for loop to find and update max error
	for (int i = 0; i < numberSubintervals; i++)
	{
		// calculate the current error
		approxErr[i] = true_solution[i] - approximate_solution[i];
        
		//compare the current error to previous maximum error and save if bigger
		if (max_error < fabs(approxErr[i]) )
		{
			max_error = approxErr[i];
		}
        
	}
   
    // find the L2 norm of the error
    double L2Norm = findL2Norm(approxErr);
    
    //output the L infinity and L2 norms to the problem info file
    ofstream fileout;
    fileout.open("problem_info.txt", std::ofstream::out | std::ofstream::app);
    fileout << " The L2 norm for the approximation error with " ;
    fileout<< numberSubintervals <<" subintervasls is " << L2Norm << ".\n";
    fileout << " The maximum error for the approximation with " ;
    fileout<< numberSubintervals <<" subintervals is " << max_error << ".\n";

    fileout.close();
    
    // return the L2 norm of the approximation error and the maximum error
